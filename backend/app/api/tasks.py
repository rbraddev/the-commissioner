...
from app.core.tasks.network_tasks import update_network_interfaces_task, deactivate_site_task, activate_site_task
from app.core.tasks.tracker import TaskTracker, create_tracker

from app.models.tasks import TaskSubmitDataNodeId, TaskSubmitDataSite, TaskSubmitted, TaskStatus
...
async def update_network_interfaces(
    ...
    # )
    await run_in_threadpool(update_network_interfaces_task(nodeids=task_data.nodeids, tracker=tracker))
...
async def deactivate_site(
    ...
    if task_data.site not in settings.TASK_DEACTIVATE_SITES:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=f"Not authorised to deactivate site: {task_data.site}")
    ...
    return {"task_id": tracker.task_id, "task_name": f"Deactivate Site {task_data.site}"}
...
async def activate_site(
    ...
    if task_data.site not in settings.TASK_DEACTIVATE_SITES:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=f"Not authorised to activate site: {task_data.site}")
